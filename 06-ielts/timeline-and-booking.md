# Timeline and booking

Diagnostic-independent. This is true regardless of what D1 scores, which is why it can be
acted on this week rather than waiting.

---

## The backwards plan

Anchored to the Swedish autumn-2027 round, not to the test.

| Date | What | Source |
|---|---|---|
| **16 Oct 2026** | universityadmissions.se opens for autumn 2027 | verified — see below |
| **15 Jan 2027** | **Application deadline** | verified |
| **1 Feb 2027** | Supporting-documents deadline | verified |
| Apr 2027 | Notification of selection results | not yet confirmed |

The **1 February documents deadline is the real backstop for the IELTS certificate**, not
15 January — the application itself closes in January, but proof of English is a supporting
document. That is roughly two extra weeks of margin.

**Do not plan around it.** Treat 15 January as the deadline and February as the thing that
saves you if something goes wrong. Reasons: the documents deadline can be applied
differently by programme, late documents can push you into a later selection round, and a
result that lands on 30 January leaves zero room for an appeal or a lost score report.

---

## Fixed points

| Date | What |
|---|---|
| **21 Aug 2026** | Today. D1 issued, unsat. |
| **4 Sept 2026** | **IELTS course starts** — length and syllabus TBC |
| 15 Jan 2027 | Swedish application closes |

## Recommended dates

> **Book this week. Sit in the last week of September — during the course, not after it.**

An earlier draft of this file said "book in the first half of August." That window has
passed; the booking is now the most overdue item in the folder, not the most comfortable.

Sitting *during* the course rather than waiting for it to finish looks impatient and is
not. Three reasons:

1. **A good result makes the course optional.** If late September returns 7.0+, the
   remaining weeks convert from obligation to insurance and can be dropped or narrowed to
   Listening only. Sitting after the course forecloses that — you pay the full time cost
   before finding out whether you needed it.
2. **A fixed date to train against.** Open-ended prep drifts; a booked date does not. A
   course start date is not a substitute — it paces the syllabus, not the exam.
3. **Three retake windows instead of one.** See below. This is the argument that does not
   bend.

The course is not the thing that earns the band. The English is already there; the gap is
exam convention, which is a two-to-three-week fix, not an eight-week one. Sequence the
course around the test, not the test around the course.

### Retake math

| | Sit | Result (CD: 3–5 days) | Still clears 15 Jan? |
|---|---|---|---|
| **Attempt 1** | last week of Sept 2026 | ~first week of Oct | yes, comfortably |
| Attempt 2 | late Oct 2026 | early Nov | yes |
| Attempt 3 | late Nov / Dec 2026 | Dec | yes |
| *(backstop)* | early Jan 2027 | mid-Jan | only against the 1 Feb documents date |

There is **no mandatory waiting period between IELTS attempts** — you can rebook as soon as
you like. The constraint is slot availability and how fast you can act on the feedback,
not a policy cooldown.

---

## The course (starts 4 Sept 2026)

### Sit D1 before it starts. This is the one genuinely time-critical item.

The course will teach the Academic Task 1 convention in its opening sessions. **Once it
does, a cold baseline is gone permanently** — there is no second first attempt. After
4 September, any Task 1 score measures how well the course was absorbed, not where the
real gap was.

The baseline is also what tells you *which parts of the course to ignore*. Without it you
sit through eight weeks of material calibrated for a B2 candidate. With it you know which
two sessions matter and can spend the rest on Listening.

### What a generic IELTS course is likely to get wrong for you

Most are built for candidates whose **English** is the obstacle. Yours is not. Expect a
meaningful share of the syllabus to be grammar drills, tense review and vocabulary
building — all of it wasted time at C2. Budget for skipping it without guilt.

What to extract instead, in priority order:

| Worth the time | Why |
|---|---|
| **Marked writing with examiner feedback** | The single highest-value thing a course provides. Ask how many scripts get individually marked, and by whom. |
| **Listening under real conditions** | The README's flagged risk, and the one thing hardest to self-administer. |
| **Reading timing technique** | 40 questions in 60 minutes. Technique, not comprehension. |
| **A mock under exam conditions** | Worth attending even if you skip everything around it. |

### Questions to put to the provider in week one

- How many writing scripts are marked individually, and is the marker a trained IELTS examiner?
- Is there a full mock under timed conditions, and when?
- Is the Listening material authentic Cambridge audio or provider-made?
- Can sessions be attended selectively?

### Open — to confirm

- **Course length and end date** (drives whether the sitting sits mid-course or after)
- Group or one-to-one; contact hours per week
- Whether it is Academic-specific or mixed Academic/General Training

⚠️ Confidentiality note: a group course is a room of people who will ask why you are taking
it. The CIM Level 7 is a true, dull, already-public answer.

Sitting in late September rather than late October is the difference between three attempts
and one. That is the whole argument for the earlier date.

---

## Booking specifics

| | |
|---|---|
| Test | **IELTS Academic** — not General Training. Selecting the wrong one wastes the fee and the slot. |
| Format | **Computer-delivered** — results in 3–5 days vs ~13 on paper. At this timeline that difference is decisive. |
| Where | Computer-delivered IELTS runs in **Abu Dhabi and Dubai**. IDP has an Abu Dhabi centre; UOWD operates in Dubai. |
| Registration closes | **4 days before the test date**, subject to availability |
| Fee | Roughly **AED 1,300–1,500** — ⚠️ *unverified for Abu Dhabi 2026; the figures found were Dubai-quoted. Confirm at booking.* |
| ID | **Passport** — must be the same passport at booking, on test day, and on the score report. A mismatch invalidates the result. |
| Results to Sweden | Results can be sent electronically. universityadmissions.se accepts IELTS Academic; confirm the exact submission route when the application opens in October. |
| Validity | 2 years — a Sept 2026 sitting covers the Jan 2027 application, the Sept 2027 intake, and a 2028 retry |

### To confirm at the point of booking
- Abu Dhabi computer-delivered dates for late September 2026 and the actual fee
- Whether the centre is IDP or British Council (both administer IELTS; either is accepted)
- The electronic score-delivery route to universityadmissions.se

---

## Buy this week

**Cambridge IELTS 19** (and 18 if available). These are the official past-paper volumes —
the only material with real difficulty calibration and, critically, **the only source of
authentic Listening audio.** D4 is blocked until one arrives.

Free YouTube practice tests are not calibrated. Scoring well on them predicts nothing, and
scoring badly on them tells you nothing either.

---

## ⚠️ Confidentiality

An IELTS Academic booking is a relocation tell — General Training is the migration test,
**Academic is the university-admission test.** Booking it says "applying to a foreign
university" more precisely than almost anything else you could do.

- Nothing on LinkedIn. No test-day post, no result, no certificate, no "brushing up on my
  English," no congratulating anyone else on theirs.
- Book with a **personal** email and personal payment card, never work.
- Test-day leave: annual leave, no stated reason.
- The Abu Dhabi centre is a small city. Assume you could be seen; have a dull, true answer
  ready — the CIM Level 7 is a genuine, non-suspicious reason to be doing formal English
  assessment, and it is already public.
